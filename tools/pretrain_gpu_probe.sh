...
REMOTE_ROOT="${SOPHIA_REMOTE_ROOT:-/root/autodl-tmp}"
...
echo "[3/5] acceleration + SDPA benchmark"
"$PYTHON_BIN" -m ml.tooling.scripts.audit_acceleration \
  --benchmark_sdpa 1 \
  --sdpa_seq_len 4096 \
  --sdpa_timed_iters "${SOPHIA_SDPA_TIMED_ITERS:-12}" \
...
for name in (
    ...
    if name.startswith("precision"):
        ...
        for row in payload.get("sdpa_benchmark", []):
            print(row)
    else:
