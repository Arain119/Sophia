...
DEFAULT_RUN_DIR = Path("runs/pretrain_e3bc2d5_bf16_qkclip")
DEFAULT_RUNS_ROOT = Path("runs")
DEFAULT_TOKENS_PER_UPDATE = 1_310_720
ERROR_PATTERNS = (
    "Traceback",
    "RuntimeError",
    "Exception",
...
RUN_START_MARKER = "[PREFLIGHT]"
@dataclass(frozen=True)
class TrainingPhase:
    """One stage of the pipeline, and the names its metrics are written under.

    Pretraining and SFT are separate trainers that grew separate vocabularies
    for the same quantities. The dashboard renders one vocabulary, so each
    phase declares how to read it rather than being special-cased at the point
    of use.
    """
...
PHASES = (
    TrainingPhase("pretrain", "Pretrain", "ml.cli.train", "max_steps"),
    TrainingPhase("sft", "SFT", "ml.cli.sft_train", "total_steps"),
)
PHASE_BY_KEY = {phase.key: phase for phase in PHASES}
TRAIN_MODULES = frozenset(phase.train_module for phase in PHASES)
...
def _load_metrics(
    ...
    for line in _read_lines(run_dir / "metrics.jsonl"):
        if not line.strip():
            continue
        try:
            row = json.loads(line)
        ...
        if row.get("type") == "start":
            ...
            train_row.setdefault(
                "tok_s",
                train_row.get("train_tokens_per_s")
                or train_row.get("tokens_per_s")
                or _row_tok_s(train_row),
            )
            # The charts read one name per quantity. Normalise here, once, so
            # that a run reporting update_seconds and max_memory_gb draws the
            # same series as one reporting tok_s and mem_gb.
            train_row.setdefault("mem_gb", train_row.get("max_memory_gb") or 0)
            latest_train = train_row
            step = row.get("step")
            if isinstance(step, int):
                history_by_step[step] = train_row
    ...
    history = [history_by_step[step] for step in sorted(history_by_step)]
    eval_history = [eval_by_step[step] for step in sorted(eval_by_step)]
    if len(history) > history_max:
        history = history[-history_max:]
    return latest_start, latest_train, history, eval_history
...
def _row_tok_s(row: dict[str, Any]) -> float:
    """Tokens per second for one update, whichever way the run reports it."""
    reported = row.get("tok_s")
    if reported:
        return float(reported)
    seconds = float(row.get("update_seconds") or 0)
    tokens = float(row.get("supervised_tokens") or 0)
    return tokens / seconds if seconds > 0 and tokens > 0 else 0.0


def _run_horizon(start: dict[str, Any], train: dict[str, Any]) -> int:
    """Total steps for the run, under whichever name its trainer writes."""
    for row in (train, start):
        for phase in PHASES:
            value = row.get(phase.horizon_key)
            if value:
                return int(value)
    return 0


def _detect_phase(run_dir: Path, start: dict[str, Any]) -> TrainingPhase:
    """Which stage this run directory belongs to."""
    for phase in PHASES:
        if start.get(phase.horizon_key):
            return phase
    name = run_dir.name.lower()
    for phase in PHASES:
        if phase.key in name:
            return phase
    return PHASES[0]


def discover_run_dir(runs_root: Path) -> Path | None:
    """The run that wrote metrics most recently.

    This is what moves the dashboard from pretraining to SFT on its own: the
    moment the next stage appends its first metrics line, it becomes the run
    with the freshest history and the page follows it.
    """
...
def resolve_launch_log(run_dir: Path) -> Path:
    """The launcher's log for a run, by the convention `<run-dir>.launch.log`."""
    return Path(f"{run_dir}.launch.log")


def _resolved_max_steps(run_dir: Path, observed_max_steps: int) -> int:
    candidates = [max(int(observed_max_steps), 0)]
    ...
    if effective_config.exists():
        ...
        candidates.append(max(int(payload.get("max_steps") or 0), 0))
    return max(candidates)


def _is_train_command(parts: list[str]) -> bool:
    return any(
        part == "-m"
        and i + 1 < len(parts)
        and parts[i + 1] in TRAIN_MODULES
...
def _current_launch_lines(launch_log: Path) -> list[str]:
    lines = _tail(launch_log, 5_000, 8_000_000)
...
def _errors(run_dir: Path, launch_log: Path) -> list[str]:
    rows: list[str] = []
    sources = (
        (launch_log, _current_launch_lines(launch_log)),
        (run_dir / "metrics.jsonl", _tail(run_dir / "metrics.jsonl", 500)),
    )
...
def _checkpoints(run_dir: Path) -> list[dict[str, Any]]:
    ...
    for path in ckpt_dir.glob("*.pt"):
        stat = path.stat()
        rows.append(
            {"name": path.name, "gb": stat.st_size / 1024**3, "mtime": stat.st_mtime}
        )
    return sorted(rows, key=lambda row: row["mtime"], reverse=True)[:5]


def _checkpoint_age_s(
    checkpoints: list[dict[str, Any]], latest_start: dict[str, Any]
) -> float | None:
    if not checkpoints:
        return None
    checkpoint_time = float(checkpoints[0]["mtime"])
    start_time = float(latest_start.get("time") or 0.0)
    return max(time.time() - max(checkpoint_time, start_time), 0.0)


def _checkpoint_write_active(run_dir: Path, *, max_age_s: float = 180.0) -> bool:
    ckpt_dir = run_dir / "checkpoints"
    if not ckpt_dir.exists():
        return False
    now = time.time()
    for path in ckpt_dir.glob("*.pt.tmp.*"):
        try:
...
def _health(
    ...
    if state == "needs_attention":
        ...
        if (
            median_tok
            and history_stats.get("tok_s", {}).get("count", 0) >= 10
            and recent_tok < 0.35 * median_tok
        ):
            health["state"] = "warning"
...
def status(run_dir: Path, launch_log: Path) -> dict[str, Any]:
    ...
    phase = _detect_phase(run_dir, latest_start)
    pids = _train_pids()
    gpu = _gpu()
    incident_path = run_dir / "stability_incident.json"
    incident = (
        incident_path.read_text(encoding="utf-8", errors="replace")
    ...
    observed_max_steps = _run_horizon(latest_start, latest_train)
    max_steps = _resolved_max_steps(run_dir, observed_max_steps)
    tok_s = float(latest_train.get("tok_s") or 0)
    remain_steps = max(max_steps - step, 0)
    seen_supervised = int(latest_train.get("seen_supervised_tokens") or 0)
    if seen_supervised > 0:
        trained_tokens = seen_supervised
        tokens_per_update = seen_supervised // max(step, 1)
    else:
    ...
    history_stats = {
        ...
        "tok_s": _series_stats(
            [r.get("tok_s") for r in history], require_positive=True
        ),
        "grad_norm": _series_stats([r.get("grad_norm") for r in history]),
    }
    recent_tok_s = (
        history_stats["tok_s"]["rolling"] or history_stats["tok_s"]["latest"] or tok_s
    )
    ...
    checkpoint_age_s = _checkpoint_age_s(checkpoints, latest_start)
    checkpoint_write_active = _checkpoint_write_active(run_dir)
    ...
    return {
        ...
        "progress": {
            ...
            "unit": "tokens",
            "trained_units": trained_tokens,
            "remain_units": remain_tokens,
            "eta_s": eta_s,
            "eta_s_recent": eta_s_recent,
            "recent_tok_s": recent_tok_s,
            "checkpoint_age_s": checkpoint_age_s,
            "checkpoint_write_active": checkpoint_write_active,
        },
        "incident": incident,
        "errors": errors,
...
<section class="panel progress-wrap">
  ...
  <div class="progress-stats">
    ...
    <div class="progress-stat">
      <div class="label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg><span id="trainedUnitLabel">已训练 Token</span></div>
      <div class="value small" id="trainedTokens">-</div>
    </div>
    <div class="progress-stat">
      <div class="label"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M19 12H5M12 19l-7-7 7-7"/></svg><span id="remainUnitLabel">剩余 Token</span></div>
  ...
  <div class="chart-card">
    ...
    <div class="chart-head">
      ...
      <div class="chart-controls">
        <span class="chart-summary" id="lossChartNote">暂无历史数据</span>
        <div class="chart-toggle" aria-label="损失数据类型">
          <button type="button" data-loss-series="train" class="active">训练</button>
          <button type="button" data-loss-series="val">验证</button>
        </div>
      </div>
    </div>
...
let lossSeriesMode='train';
let gradScaleMode='body';
function renderCharts(d){
  const hist=(d.history||[]).filter(r=>!r.type||r.type==='train');
  const stats=d.history_stats||{};

  const trainLossSeries=hist.map(r=>({x:r.step,y:Number(r.loss),time:r.time,lr:r.lr,mem_gb:r.mem_gb})).filter(p=>Number.isFinite(p.y));
  const valLossSeries=(d.eval_history||[]).map(r=>({x:r.step,y:Number(r.val_loss),time:r.time})).filter(p=>Number.isFinite(p.y));
  const showValLoss=lossSeriesMode==='val';
  const lossSeries=showValLoss?valLossSeries:trainLossSeries;
  const lossRolling=showValLoss?null:movingAvg(lossSeries,8);
  const maxStep=(d.progress&&d.progress.max_steps)||((hist[hist.length-1]||{}).max_steps)||0;
  const xEnd=((hist[hist.length-1]||{}).step)||maxStep||0;
  drawSmoothChart($('lossChart'),lossSeries,{label:showValLoss?'验证损失':'训练损失',color:showValLoss?'#3b4f68':'#a65d2e',fill:showValLoss?'rgba(59,79,104,.18)':'rgba(166,93,46,.22)',decimals:4,rolling:lossRolling,rollingColor:'rgba(46,41,30,.5)',zeroBaseline:true,xStart:0,xEnd});
  const lossStats=stats.loss||{};
  const bestVal=valLossSeries.length?Math.min(...valLossSeries.map(p=>p.y)):null;
  $('lossChartNote').textContent=lossSeries.length>=2?(showValLoss?`最佳 ${bestVal.toFixed(4)} · ${valLossSeries.length} 次验证`:`滚动均值 ${lossStats.rolling?lossStats.rolling.toFixed(4):'-'}`):'暂无足够历史数据';

  if(showValLoss && valLossSeries.length===1){
    const onlyVal=valLossSeries[0];
    $('lossChartNote').textContent=`val loss ${onlyVal.y.toFixed(4)} @ step ${onlyVal.x}`;
  }
...
async function refresh(){
  ...
  const phase=d.phase||{};
  $('run').textContent=phase.label?`${phase.label} · ${d.run_dir||'-'}`:(d.run_dir||'-');
  ...
  $('progressSub').textContent=max?`步数 ${nf.format(step)} / ${nf.format(max)}`:'暂无训练进度数据';
  $('step').textContent=(step===0&&max===0)?'暂无':nf.format(step);
  $('remainSteps').textContent=max?nf.format(p.remain_steps):'暂无';
  const usesExamples=p.unit==='examples';
  $('trainedUnitLabel').textContent=usesExamples?'已训练样本':'已训练 Token';
  $('remainUnitLabel').textContent=usesExamples?'剩余样本':'剩余 Token';
  $('trainedTokens').textContent=usesExamples?nf.format(p.trained_units||0):fmtTokens(p.trained_tokens);
  $('remainTokens').textContent=max?(usesExamples?nf.format(p.remain_units||0):fmtTokens(p.remain_tokens)):'暂无';
...
document.addEventListener('click',event=>{
  const lossBtn=event.target&&event.target.closest?event.target.closest('[data-loss-series]'):null;
  if(lossBtn){
    lossSeriesMode=lossBtn.dataset.lossSeries==='val'?'val':'train';
    document.querySelectorAll('[data-loss-series]').forEach(x=>x.classList.toggle('active',x===lossBtn));
...
class Handler(BaseHTTPRequestHandler):
    ...
    def active_run(self) -> tuple[Path, Path]:
        """Re-resolve on every poll so a new stage is picked up without a restart."""
        if self.pinned or self.runs_root is None:
            return self.run_dir, self.launch_log
        discovered = discover_run_dir(self.runs_root)
        if discovered is None:
            return self.run_dir, self.launch_log
        return discovered, resolve_launch_log(discovered)

    def log_message(self, fmt: str, *args: object) -> None:
        return

    def send_body(
        self, body: bytes, content_type: str, code: HTTPStatus = HTTPStatus.OK
    ) -> None:
    ...
    def do_GET(self) -> None:
        if self.path == "/":
            ...
            run_dir, launch_log = self.active_run()
            payload = status(run_dir, launch_log)
            self.send_body(
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="follow whichever run under this root is currently writing metrics",
    )
    ...
    # A pinned --run-dir stays pinned; otherwise the page follows the pipeline.
    Handler.pinned = args.run_dir is not None
    Handler.runs_root = args.runs_root
    run_dir = args.run_dir or discover_run_dir(args.runs_root) or DEFAULT_RUN_DIR
    Handler.run_dir = run_dir
    Handler.launch_log = args.launch_log or resolve_launch_log(run_dir)
        ...
        json.dumps(
            {
                ...
                "run_dir": str(run_dir),
                "follows_pipeline": not Handler.pinned,
            },
            ensure_ascii=False,
        ),
        flush=True,
