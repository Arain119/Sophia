...
import ml.training.pretrain.observability as observability_mod
...
PINNED_MUON_NS_STEPS = int(RELEASE_PRETRAIN_DEFAULTS.muon_ns_steps)


def _release_machine_int(field_name: str, *, default: int = 0) -> int:
    raw = DEFAULT_PRETRAIN_MACHINE_RECIPE.get(str(field_name), default)
    return int(raw)


def _release_runtime_int(field_name: str, *, default: int = -1) -> int:
    raw = DEFAULT_PRETRAIN_MACHINE_RUNTIME.get(str(field_name), default)
    return int(raw)
...
def build_pretrain_args(
    *,
    ...
    machine_recipe_migration_from_sha256: str = "",
    allow_protocol_path_relocation: int = 0,
    ...
    return PretrainRunConfig(
        ...
        machine_recipe_json=str(machine_recipe_json or "").strip(),
        _sophia_machine_recipe_migration_from_sha256=(
            str(machine_recipe_migration_from_sha256 or "").strip()
        ),
        _sophia_allow_protocol_path_relocation=int(allow_protocol_path_relocation),
        ...
        total_tokens=int(active_profile.training.default_total_tokens),
        ...
        muon_ns_steps=PINNED_MUON_NS_STEPS,
        ...
        shard_preload=_release_runtime_int("shard_preload") if release_profile else -1,
        shard_preload_bytes=_release_runtime_int("shard_preload_bytes")
        if release_profile
        else -1,
        step_execution_backend=(
            str(DEFAULT_PRETRAIN_MACHINE_RUNTIME["step_execution_backend"])
            if release_profile
            else ""
        ),
        max_seq_len=int(active_profile.training.max_seq_len),
        seq_len=int(active_profile.training.train_seq_len),
...
def run(args: PretrainRunConfig) -> dict[str, object]:
    pipeline = PretrainPipeline(args=args)
    pipeline.run()
    return dict(getattr(pipeline.runner, "runtime_evidence", {}))


def run_preflight(args: PretrainRunConfig) -> None:
    """Run release preflight through data, recipe, and model loading only."""

    pipeline = PretrainPipeline(args=args)
    pretrain_runtime.setup_runtime(
        pipeline,
        deps=_pretrain_deps(),
        machine_signature_arg_key=pipeline._MACHINE_SIGNATURE_ARG_KEY,
    )
    pretrain_runtime.run_runtime_preflight(pipeline, deps=_pretrain_deps())
    pretrain_runtime.prepare_data_and_model(pipeline, deps=_pretrain_deps())
    print(
        "[PREFLIGHT] release data, recipe, machine, and model checks passed", flush=True
    )
