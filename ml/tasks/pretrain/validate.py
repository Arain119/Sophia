...
class PretrainValidationConfig:
    data_path: str = "dataset/pretrain_tokens"
    tokenizer_path: str = ""
    output_dir: str = ""
...
def pretrain_validation_config_from_args(args: object) -> PretrainValidationConfig:
    ...
    return PretrainValidationConfig(
        data_path=str(payload.get("data_path", "dataset/pretrain_tokens")),
...
def build_validation_args(config: PretrainValidationConfig) -> PretrainRunConfig:
    ...
    return replace(
        ...
        # Validation carries no machine recipe, so stage planning needs an
        # explicit positive micro-batch budget (batch_size=1 per micro-step).
        target_tokens_per_microbatch=int(args.seq_len),
        warmup_steps=0,
        warmup_ratio=0.1,
        total_tokens=(
