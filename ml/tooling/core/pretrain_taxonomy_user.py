...
_RE_NSFW = re.compile(
    r"(无码(AV片|视频|影片)|AV片|成人视频|成人色情|"
    r"色情(视频|影片|网站|图片|小说)|"
    r"乱伦(视频|影片|色情|小说)|兽交(视频|影片|色情)|"
    r"(口交|肛交)(视频|影片)|性奴(调教|视频|影片)|SM调教|"
    r"\b(porn|porno|sex\s+video|hardcore\s+porn)\b)",
    re.IGNORECASE,
)
...
def infer_user_taxonomy_label(text: str) -> str:  # noqa: C901
    ...
    legal_hits = sum(1 for _ in _RE_LEGAL_ANY.finditer(t[:20000]))
    if legal_hits:
