...
def _resume_report_check(*, path: Path) -> GateCheck:
    ...
    passed = (
        ...
        and payload.get("stage") == "pretrain"
...
def build_soak_test_audit(
    *,
    ...
    if not resume_report_paths:
        ...
        resume_checks = [
            _resume_report_check(path=Path(str(path)).resolve())
            for path in resume_report_paths
        ]
    ...
    return {
        ...
        "resume_reports": [
            str(Path(str(path)).resolve()) for path in resume_report_paths
        ],
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Stage/run mapping in the form pretrain=RUN_DIR.",
    )
    parser.add_argument(
        "--resume_report",
        type=str,
