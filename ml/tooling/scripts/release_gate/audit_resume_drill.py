...
def _artifact_check(*, stage: str, run_dir: Path) -> GateCheck:
    required = ("metrics.jsonl", "run_args.json", "run_meta.json", "tensorboard", "checkpoints")
    if str(stage) == "sft":
        required = (*required, "machine_recipe_summary.json")
...
def _checkpoint_contract_checks(*, stage: str, run_dir: Path) -> list[GateCheck]:
    ...
    resolved_resume = str(args_payload.get("_sophia_resolved_resume_checkpoint", "") or "").strip()
    checks.append(
        GateCheck(
            name="checkpoint_resume_marker_present",
            passed=bool(resolved_resume),
            detail=(resolved_resume or f"missing _sophia_resolved_resume_checkpoint in {checkpoint_path}"),
        )
    )
    train_state = checkpoint.train_state if isinstance(checkpoint.train_state, dict) else None
    checks.append(
        GateCheck(
            name="checkpoint_train_state_present",
    ...
    checks.append(_stage_train_state_contract_check(stage=str(stage), train_state=train_state, checkpoint_path=checkpoint_path))
    return checks
...
def _stage_train_state_contract_check(
    *,
    ...
    if str(stage) == "sft":
        required = ("stage", "train_iter_state", "eval_iter_state", "test_iter_state", "samples_seen")
        missing = [key for key in required if key not in train_state]
        passed = not missing and str(train_state.get("stage", "") or "") == "sft"
        return GateCheck(
            name="stage_train_state_contract",
            passed=passed,
            detail=(
                "sft iterator + samples_seen state captured"
                if passed
                else f"missing_or_invalid keys in {checkpoint_path}: {', '.join(missing or ['stage'])}"
            ),
        )
...
def build_resume_drill_audit(
    *,
    ...
    if missing_scenarios:
        raise ValueError(
            "missing required scenarios: " + ", ".join(str(item) for item in missing_scenarios)
        )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Audit machine-level resume drill evidence for a release training stage."
    )
    parser.add_argument(
        "--stage",
        type=str,
