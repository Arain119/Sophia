...
def validate_generate_request(
    *,
    ...
    del eos_id
    if int(prefix_cache_min_tokens) < 1:
        raise ValueError(
            f"prefix_cache_min_tokens must be >= 1, got {prefix_cache_min_tokens}"
        )
    if prefill_chunk_size is not None and int(prefill_chunk_size) <= 0:
        raise ValueError(
            f"prefill_chunk_size must be > 0 when provided, got {prefill_chunk_size}"
        )
    ...
    if prefix_cache_enabled:
        model_prefix_cache_namespace(model)
...
def normalize_generation_inputs(
    *,
    ...
    prefix_cache_enabled = prefix_cache is not None or prefix_cache_dir is not None
    if prefix_cache_enabled and batch_size != 1:
        raise ValueError("prefix caching only supports batch_size == 1 in this helper")
