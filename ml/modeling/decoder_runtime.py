...
class DecoderRuntimeBase:
    ...
    def prefix_cache_namespace(self) -> str:
        explicit = getattr(self, "_prefix_cache_namespace", None)
        if isinstance(explicit, str) and explicit.strip():
            return explicit
        namespace = str(
            self._runtime_model().prefix_cache_namespace or ""
        ).strip()
        if not namespace:
            raise ValueError(
                "decoder runtime requires an explicit prefix_cache_namespace"
            )
        return namespace
...
class DecoderRuntimeMixin(DecoderRuntimeBase):
    ...
    def cache_dump_prefix(
        ...
        *,
        ...
        return self._runtime_host().cache_dump_prefix(
            device=device,
    ...
    def runtime_prefix_replay_len(self) -> int:
        return int(self._runtime_host().runtime_prefix_replay_len())
...
class DecoderModelMixin(DecoderRuntimeBase):
    ...
    def _prepare_runtime_state_dict_for_load(
        self,
    ...
    def load_state_dict(
        ...
        with self._model_runtime_lock:
            ...
            result = super().load_state_dict(
                self._prepare_runtime_state_dict_for_load(state_dict),
                strict=strict,
                assign=assign,
            )
            self._sync_tied_runtime_word_embeddings()
...
class DecoderRecipeMixin(DecoderRuntimeBase):
    ...
    def apply_runtime_recipe_knobs(
        ...
        *,
        ...
        with self._model_runtime_lock:
            chunk_size = (
                max(int(loss_chunk_size), 0) if self.supports_loss_chunk_size() else 0
            )
            exclude_first = (
                int(gradient_checkpointing_exclude_first)
                if self.supports_checkpoint_excludes()
                else 0
            )
            exclude_last = (
