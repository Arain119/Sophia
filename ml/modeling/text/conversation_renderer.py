...
def assistant_payload(message: ConversationMessage) -> str:
    ...
    if tool_calls is not None:
        ...
        parts.extend(render_tool_call(call) for call in tool_calls)
    if not bool(message.get("wo_eos")):
        parts.append(EOS_TEXT)
    return "".join(parts)


def assistant_is_masked(message: ConversationMessage) -> bool:
    value = message.get("mask", False)
    if not isinstance(value, bool):
        raise TypeError("assistant.mask must be a boolean when provided")
    return value
...
def transition_after_message(
    ...
    *,
    ...
    need_transition = bool(
        is_last or next_role in {"assistant", "latest_reminder", "tool"}
    )
    role = str(message.get("role", "")).strip()
    if need_transition and role in {"user", "developer", "tool"} and not (
    ...
    *,
    ...
    for index, message in enumerate(prepared):
        ...
        if role in {"system", "developer"}:
            ...
            if body:
                segments.append(
                    RenderedSegment(body, not assistant_is_masked(message))
                )
        elif role == "tool":
            segments.append(RenderedSegment(render_tool_result(message.get("result")), False))
        else:
