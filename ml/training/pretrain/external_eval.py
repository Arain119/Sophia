...
def _sample_split_contamination(
    *,
    ...
    safe_rate = 1.0
    if int(rows) > 0:
        safe_rate = 1.0 - (float(flagged_count) / float(rows))
    return {
        ...
        "flagged_samples": int(flagged_count),
...
def run_contamination_scan_eval(
    *,
    ...
    payload = {
        ...
        "flagged_count": int(sum(int(result["flagged_count"]) for result in results)),
        "flagged_examples_count": int(sum(len(result["examples"]) for result in results)),
    }
