...
def maybe_resume_from_checkpoint(
    *,
    ...
    optimizer_kind = str(args.optimizer_kind or TORCH_MUON_HYBRID)
    ...
    if optimizer_kind != TORCH_MUON_HYBRID:
        raise SophiaUsageError(
            f"[ERR] unsupported pretrain optimizer_kind={optimizer_kind!r}"
        )
    ...
    try:
        ...
        raise SophiaUsageError(
            f"[ERR] failed to load optimizer state for {optimizer_kind} checkpoint: {exc}"
        ) from exc
    move_optimizer_state_to_device_fn(optimizer, str(device))
    print(f"[OPT] optimizer={optimizer_kind} (resumed)", flush=True)
