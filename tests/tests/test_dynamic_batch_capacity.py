...
def test_model_args_runtime_controls_use_explicit_methods() -> None:
    ...
    with pytest.raises(ValueError, match="cannot exceed configured max_seq_len"):
        args.ensure_runtime_sequence_capacity(96)
    assert int(args.max_seq_len) == 64
    assert args.runtime_sequence_capacity() == 64


def test_model_args_runtime_controls_reject_non_positive_capacity() -> None:
    args = ModelArgs(max_batch_size=1, max_seq_len=64)

    with pytest.raises(ValueError, match="batch_size must be > 0"):
        args.ensure_runtime_batch_capacity(0)
    with pytest.raises(ValueError, match="max_seq_len must be > 0"):
        args.ensure_runtime_sequence_capacity(-1)
...
def test_runtime_no_cache_model_full_sequence_eval_stays_stateless_cpu(
    ...
    for layer in model.layers:
        if layer.layer_type == "kda":
            assert torch.count_nonzero(layer.attn.recurrent_state) == 0
            assert torch.count_nonzero(layer.attn.conv_state) == 0
        else:
            assert torch.count_nonzero(layer.attn.latent_cache) == 0
...
def test_runtime_no_cache_model_replay_enables_cache_cpu() -> None:
    ...
    assert int(model.layers[0].attn.recurrent_state.size(0)) >= 1
