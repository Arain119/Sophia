...
def test_resolve_runtime_prefers_explicit_contract() -> None:
    ...
    assert resolve_runtime_model(wrapper) is runtime_model
...
def test_resolve_runtime_accepts_runtime_model_directly() -> None:
    ...
    assert resolve_runtime_model(runtime_model) is runtime_model
...
def test_decoder_runtime_mixin_delegates_generation_methods() -> None:
    ...
    class _RuntimeModel:
        ...
        prefix_cache_namespace = "test.runtime"
    ...
    assert wrapper.cache_dump_prefix(device="cpu", prefix_len=8) == (
        "dump_prefix",
        (),
        {"device": "cpu", "prefix_len": 8, "batch_size": None},
    )
    assert wrapper.runtime_max_seq_len() == 128
    assert wrapper.runtime_prefix_replay_len() == 32
