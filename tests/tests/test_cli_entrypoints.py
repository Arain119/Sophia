...
from ml.cli import pretrain_check as pretrain_check_cli
from ml.tooling import cli as tooling_cli


def test_tooling_cli_propagates_target_return_code(monkeypatch) -> None:
    monkeypatch.setattr(tooling_cli, "ensure_standard_stack", lambda **_kwargs: None)
    monkeypatch.setattr(
        tooling_cli.importlib,
        "import_module",
        lambda _name: SimpleNamespace(main=lambda: 7),
    )

    assert tooling_cli._run_target("fake:main", [], prog="fake") == 7
...
def test_pretraining_cli_defaults_use_canonical_dataset_root() -> None:
    assert Path(pretrain_check_cli.build_parser().parse_args([]).data_path) == Path(
        "dataset/pretrain"
    )
