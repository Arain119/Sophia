...
def test_train_loop_stability_guard_stops_and_writes_incident() -> None:
    model = _ExportableToyModel(1, 1, bias=False)
    with torch.no_grad():
        model.weight.zero_()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        cfg = LoopConfig(
            ...
            stability_guard_loss_window=1,
            stability_guard_loss_max=0.5,
            stability_guard_min_step=1,
        )
        ...
        assert incident["reason"] == "loss_exceeded"
        assert int(incident["step"]) == 1


def test_train_loop_stability_guard_stops_on_single_grad_spike() -> None:
    model = _ExportableToyModel(1, 1, bias=False)
    with torch.no_grad():
        model.weight.fill_(3.0)
    optimizer = torch.optim.SGD(model.parameters(), lr=0.0)
    runner = _FixedGradRunner(model, loss=0.25, grad=2.0)
    data_iter = _ToyDataIter()
    ...
    with tempfile.TemporaryDirectory() as tmp:
        cfg = LoopConfig(
            ...
            enable_checkpoints=0,
            stability_guard_enabled=1,
            stability_guard_grad_norm_max=1.5,
            stability_guard_min_step=1,
        )
        with pytest.raises(RuntimeError, match="stability guard tripped"):
            train_loop(
        ...
        with open(incident_path, encoding="utf-8") as f:
            incident = json.load(f)
        assert incident["reason"] == "grad_norm_exceeded"
        assert int(incident["step"]) == 1
        assert float(incident["grad_norm"]) == pytest.approx(2.0)
        assert float(incident["pre_clip_grad_norm"]) == pytest.approx(2.0)
        assert float(incident["post_clip_grad_norm"]) == pytest.approx(1.0)
