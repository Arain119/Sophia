...
def test_init_model_uses_context_window_for_tokenizer(monkeypatch, tmp_path) -> None:
    ...
    model = types.SimpleNamespace(
        config=types.SimpleNamespace(max_seq_len=4096),
    ...
    assert model_kwargs["gradient_checkpointing"] is False
    assert model_kwargs["gradient_checkpointing_exclude_first"] == 0
    assert model_kwargs["gradient_checkpointing_exclude_last"] == 0
