...
def test_write_remote_code_bundle_writes_modeling_sophia() -> None:
    ...
    from ml.integrations.export.runtime_packager_manifest import (
        ...
        EXPORT_DECODER_CORE_FILENAME,
        EXPORT_DECODER_OUTPUT_FILENAME,
        EXPORT_DECODER_HOST_COMPAT_FILENAME,
        EXPORT_DECODER_RUNTIME_MODEL_FILENAME,
        EXPORT_DECODER_RUNTIME_RECIPE_FILENAME,
        EXPORT_DECODER_RUNTIME_FILENAME,
        EXPORT_SOPHIA_DECODER_FILENAME,
        EXPORT_PRETRAINED_BUNDLE_FILENAME,
        EXPORT_ROPE_FILENAME,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert os.path.exists(os.path.join(td, EXPORT_ROPE_FILENAME))
        ...
        assert os.path.exists(os.path.join(td, EXPORT_DECODER_CORE_FILENAME))
        assert os.path.exists(os.path.join(td, EXPORT_DECODER_OUTPUT_FILENAME))
        assert os.path.exists(
            os.path.join(td, EXPORT_DECODER_HOST_COMPAT_FILENAME)
        )
        assert os.path.exists(
            os.path.join(td, EXPORT_DECODER_RUNTIME_MODEL_FILENAME)
        )
        assert os.path.exists(
            os.path.join(td, EXPORT_DECODER_RUNTIME_RECIPE_FILENAME)
        )
        assert os.path.exists(
...
def test_hf_config_metadata_marks_default_high_theta_rope_recipe() -> None:
    cfg = SophiaConfig()

    # Decoder uses high-theta RoPE: original_seq_len == 0 keeps
    # HF on the plain "default" recipe.
    assert int(cfg.original_seq_len) == 0
    assert cfg.rope_parameters["rope_type"] == "default"
    assert float(cfg.rope_parameters["rope_theta"]) == 500000.0
...
def test_hf_model_preserves_depth_scaled_residual_init() -> None:
    ...
    residual_std = 0.02 / ((2.0 * float(cfg.n_layers)) ** 0.5)
