...
def test_top_level_packages_expose_stable_subpackages() -> None:
    ...
    assert importlib.import_module("ml.training.posttrain") is not None
    assert runtime is not None
    assert importlib.import_module("ml.runtime.inference") is not None
    assert callable(runtime.resolve_runtime_model)
...
def test_training_package_exports_stable_entrypoints() -> None:
    ...
    assert callable(importlib.import_module("ml.cli.rehearsal").main)
    assert callable(importlib.import_module("ml.cli.sft").main)
    ...
    assert callable(importlib.import_module("ml.tasks.sft.train").run)
