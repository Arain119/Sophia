...
def test_build_pretrain_release_semantics_captures_optimizer_and_clip_semantics(
    tmp_path,
) -> None:
    ...
    assert float(semantics["adam_eps"]) == pytest.approx(1e-6)
    assert int(semantics["muon_ns_steps"]) == 4
    assert float(semantics["muon_target_rms"]) == pytest.approx(0.18)
    assert float(semantics["embedding_lr_scale"]) == pytest.approx(0.25)
    assert float(semantics["ema_decay"]) == pytest.approx(0.0)
    assert str(semantics["grad_clip_mode"]) == "hybrid_auto"
    assert float(semantics["agc_clip"]) == pytest.approx(0.01)
    assert float(semantics["agc_eps"]) == pytest.approx(1e-3)
    assert int(semantics["agc_exclude_bias_and_norm"]) == 1


def test_release_pretrain_preflight_applies_recipe_and_materializes_artifacts(tmp_path) -> None:
    args, bootstrap = _bootstrap(tmp_path)
    machine_signature = dict(args._sophia_machine_signature or {})
    recipe_payload = {
        "kind": PRETRAIN_MACHINE_RECIPE_KIND,
        "stage": "pretrain",
        "selected_name": CANONICAL_PRETRAIN_MACHINE_RECIPE_NAME,
        ...
        "machine_recipe": dict(CANONICAL_PRETRAIN_MACHINE_RECIPE_PAYLOAD),
        "machine_runtime": dict(CANONICAL_PRETRAIN_MACHINE_RUNTIME_PAYLOAD),
        "artifacts": {
            "machine_recipe": {"best": {"batch_size": 1}},
            "machine_runtime": {"best": {"tokens_per_sec": 123.0}},
            "update_profile": {"tokens_per_sec_est": 27334.2, "tokens_per_update": 270336},
        },
    }
    ...
    assert int(resolved.batch_size) == 3
    assert int(resolved.accumulation_steps) == 22
    ...
    assert int(resolved.shard_preload_bytes) == 4194304
...
def test_release_pretrain_preflight_rejects_memory_signature_mismatch(
    ...
    recipe_signature = dict(args._sophia_machine_signature or {})
    current_signature = dict(args._sophia_machine_signature or {})
    current_signature["cuda_device_name"] = "NVIDIA CUDA GPU"
    current_signature["cuda_capability"] = [9, 0]
